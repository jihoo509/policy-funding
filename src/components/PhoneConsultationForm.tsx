      ...
      >
        ...
        <form onSubmit={handleSubmit} className="space-y-3">
          ...
          <div className="space-y-2">
            ...
            <div className="flex h-12 bg-white rounded-md overflow-hidden">
              <Button type="button" onClick={() => handleInputChange('isLoanOverdue', '예')} className={`flex-1 rounded-none h-full border-0 ${formData.isLoanOverdue === '예' ? 'bg-[#f59e0b] text-white' : 'bg-white text-gray-600'}`}>예</Button>
              <Button type="button" onClick={() => handleInputChange('isLoanOverdue', '아니오')} className={`flex-1 rounded-none h-full border-0 ${formData.isLoanOverdue === '아니오' ? 'bg-[#f59e0b] text-white' : 'bg-white text-gray-600'}`}>아니오</Button>
            </div>
          </div>
          <div className="space-y-2">
            <label className="text-white text-base block">정책자금 신청 진행 여부</label>
            <div className="flex h-12 bg-white rounded-md overflow-hidden">
              <Button type="button" onClick={() => handleInputChange('hasAppliedForPolicyFund', '예')} className={`flex-1 rounded-none h-full border-0 ${formData.hasAppliedForPolicyFund === '예' ? 'bg-[#f59e0b] text-white' : 'bg-white text-gray-600'}`}>예</Button>
              <Button type="button" onClick={() => handleInputChange('hasAppliedForPolicyFund', '아니오')} className={`flex-1 rounded-none h-full border-0 ${formData.hasAppliedForPolicyFund === '아니오' ? 'bg-[#f59e0b] text-white' : 'bg-white text-gray-600'}`}>아니오</Button>
            </div>
          </div>
          ...
          <div className="pt-2">
            <Button type="submit" disabled={!formData.name || !formData.birthDate || !formData.gender || !formData.phoneNumber || !formData.companyName || !formData.businessNumber || !formData.existingLoanStatus || !formData.isLoanOverdue || !formData.hasAppliedForPolicyFund || !formData.agreedToTerms || isSubmitting} className="w-full h-14 bg-[#f59e0b] hover:bg-[#d97706] text-white text-xl disabled:opacity-50">
